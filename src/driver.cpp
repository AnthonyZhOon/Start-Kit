...
void save_results(
    const std::string &fileName, const base_system::metrics_t &system_metrics,
  ...
  file << "Planning Time (ns): "
       << std::chrono::duration_cast<std::chrono::nanoseconds>(
              planner_metrics.planning_time_)
              .count()
       << "\n";

  // Write task generator metrics
  file << "\nTask Generator Metrics:\n";
  file << "Number of Tasks Finished: "
       << task_generator_metrics.num_of_task_finish << "\n";
...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help", "produce help message")(
      "inputFolder", po::value<std::string>()->default_value("./"),
      "inputfolder")("inputFile,i", po::value<std::string>()->required(),
                     "input file name")(
      ...
      "evaluate an existing output file")("simulationTime",
                                          po::value<int>()->default_value(5000),
                                          "run simulation")(
      "fileStoragePath", po::value<std::string>()->default_value(""),
      "the path to the storage path")("planTimeLimit",
                                      po::value<int>()->default_value(INT_MAX),
  ...
  planner::MAPFPlannerWrapper wrapped_planner(
      &planner, &logger, std::chrono::seconds(preprocess_time_limit),
      std::chrono::seconds(plan_time_limit));
  ...
  save_results(output_file, system_metrics, planner_metrics,
               task_generator_metrics);
}
